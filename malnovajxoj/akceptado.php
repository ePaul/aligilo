...
HtmlKapo();
{
  ...
  eoecho ("Ni nun akceptas <b>".$partoprenanto->datoj[personanomo]." ".
		  $partoprenanto->datoj[nomo]." </b>(".$partoprenanto->datoj[ID].
		  ") al la <b>".$_SESSION["renkontigxo"]->datoj[nomo]."</b>");
  ...
  eoecho ("<br />\nSe jes,"); ligu("partrezultoj.php", "s^angu ilin");
  ...
  if ($partopren->datoj['agxo']<=26) // TODO: prenu limagxon el datumbazo (aux cxiam kontrolu)
	{
	  eoecho ("<li>Eventuale kontrolu la ag^on / naskig^daton. (" . 
			  $partopreno->datoj[agxo]." / ".$partoprenanto->datoj[naskigxdato] .
			  ")</li>\n");
	}
  ...
  if ($partopreno->datoj['agxo']<18)
	{
	  eoecho ("<li>Kolektu la gepatran permeson (se g^i mankas donu faksnumeron de" .
			  " la ejo kaj insistu ke $ri donos).</li>\n");
	}
  ...
  echo "<li>\n";
  if (deviga_membreco_tipo=='landa' and ($partoprenanto->datoj["lando"] == HEJMLANDO )) // Hejmlando = normale Germanio
	{
