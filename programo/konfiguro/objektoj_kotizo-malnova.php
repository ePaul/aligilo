...
class Kotizo
{
  ...
  function kalkulu_aligx_kategorio($relevantadato,$renkontigxo)
  {
	/* if ( $landakategorio[0] == "C" ) $io=$partopreno->datoj[aligxdato];
       else if (( $landakategorio[0] == "B" ) and ($partopreno->datoj[antauxpago]>=10))  $io=$partopreno->datoj[antauxpagdato];
       else if (( $landakategorio[0] == "A" ) and ($partopreno->datoj[antauxpago]>=30))  $io=$partopreno->datoj[antauxpagdato];
       else return 0;
  ...
  /*function aligx_kato($io,$renkontigxo,&$limdato)
{
  ...
  */
  ...
  function formatu_agxkategorion($renkontigxo)
  {
	if ($renkontigxo->datoj["ID"] < 4)
	  {
		if ($this->agxkategorio==0)
		  {
			return "> ".$renkontigxo->datoj[maljuna];
		  }
		elseif  ($this->agxkategorio==1)
		  {
			return "> ".$renkontigxo->datoj[juna];
		  }
		elseif ($this->agxkategorio==2)
		  {
			return "<= ".$renkontigxo->datoj[juna];
		  }
	  }
