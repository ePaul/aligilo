...
function MultiCell($w,$h,$txt,$border=0,$align='J',$fill=0)
{
	...
	while($i<$nb)
	{
		...
		// varianto de PHPMyAdmin (laboras kun novaj tipar-difinoj)
		//		$l+=isset($cw[ord($c)])?$cw[ord($c)]:0;
		// varianto el la ufpdf-elsxuto
		//		$l+=$cw[$c];
		// mia varianto (laboras kun la malnovaj tipar-difinoj):
		// $l+=isset($cw[($c)])?$cw[($c)]:0;
		// nova provo - eble tio funkcias ambauxmaniere ...
		$l += isset($cw[ord($c)]) ? $cw[ord($c)] : $cw[$c];
...
function Write($h,$txt,$link='')
{
	...
	while($i<$nb)
	{
		...
		if($c==' ')
		  $sep=$i;
		// kopio de la samspeca sxangxo en multiCell() -- Pauxlo
		$l+=isset($cw[ord($c)])?$cw[ord($c)]:$cw[$c];
		// originalo:
		//		$l+=$cw[$c];
		if($l>$wmax)
