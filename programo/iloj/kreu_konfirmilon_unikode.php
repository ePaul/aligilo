...
class Konfirmilo
{
  ...
  function Konfirmilo()
  {
	...
	//	$this->pdf->AddFont("Arial",'I',"helveticai.php");
									  ...
									  $renkontigxo, $kotizo, $lingvo)
  {
	...
	$this->pdf->text(105,34, utf8_encode(".... wir machen Völkerverständigung"));
	...
	$this->pdf->setY(59);
	$this->pdf->write(5, uni($partoprenanto->datoj[personanomo]." ".$partoprenanto->datoj[nomo]));
	$this->pdf->ln();
	if ($partoprenanto->datoj[adresaldonajxo]!='')
	  {
		$this->pdf->write(5,uni($partoprenanto->datoj[adresaldonajxo]));
		$this->pdf->ln();
	  }
	$this->pdf->write(5,uni($partoprenanto->datoj[strato]));
	$this->pdf->ln();
	$this->pdf->write(5,uni($partoprenanto->datoj[posxtkodo]." ".$partoprenanto->datoj[urbo]));
	$this->pdf->ln();
	$this->pdf->write(5,uni(eltrovu_landon_lokalingve($partoprenanto->datoj[lando])));
	...
	$this->pdf->write(5, uni(donu_tekston_lauxlingve("konf2-enkonduko",
													$lingvo, $renkontigxo)));
	...
	if ($kotizo->landakategorio=='C')
	  ...
	  $this->pdf->cell(40,4,uni("Alveno de via antau^pago:",
							   "Ankunft der Anzahlung:", $lingvo),0,2,'R');
 
	$this->pdf->cell(40,4,uni("Via log^landa kategorio:",
							 "Deine Landeskategorie", $lingvo), 0, 2, 'R');
	$this->pdf->cell(40,4,uni("Via ag^kategorio:",
							 "Deine Alterskategorie", $lingvo), 0, 2, 'R');
	$this->pdf->cell(40,4,uni("Partoprentagoj:",
	...
	$this->pdf->cell(40,4,uni("Memzorganto:",
							 "Selbstversorger:"),0,2,'R');
	$this->pdf->cell(40,4,uni("Dulita c^ambro:",
	...
	$this->pdf->cell(40,4,uni($alk),0,2,'L');
	$this->pdf->cell(40,4,uni($kotizo->landakategorio),0,2,'L');
	...
	$this->pdf->cell(40,4,uni($aka),0,2,'L');
	$this->pdf->cell(40,4,uni($kotizo->partoprentagoj),0,2,'L');
	...
	$this->pdf->write(4, uni($teksto));
	$this->pdf->ln(8);
	//$this->pdf->setY(155);
	$this->pdf->SetFont($this->font.'D','',11);
	...
	$teksto.=' ';
	$this->pdf->multicell(170,4, uni($teksto));
	...
	$this->pdf->write(5, uni(donu_tekston_lauxlingve("konf2-elkonduko",
													 $lingvo, $renkontigxo)));
	...
	$this->pdf->multicell(170,5, uni($enhavo));
  ...
  function kreu_permesilon($partoprenanto, $renkontigxo, $defVira = "")
  {
	...
	$this->pdf->SetFont($this->font,'',14);
	$this->pdf->cell(160,10,utf8_encode("(Nur für Teilnehmer, die am " .
